...
class QvdFile:
    def __init__(self, name):
        ...
        self.mode = (
            os.O_RDONLY
            if os.path.split(name)[1].split(".")[1].lower() == "qvd"
            else os.O_WRONLY
        )
        ...
        self.attribs = self.root[
            "QvdTableHeader"
        ]  # dict of attributes - save some typing
        self.fields = self.root["QvdTableHeader"]["Fields"][
            "QvdFieldHeader"
        ]  # list of fields - save typing
    ...
    def getRow(self, rInd):
        ...
        os.lseek(
            self.fp,
            self.stPos
            + int(self.attribs["Offset"])
            + int(self.attribs["RecordByteSize"]) * rInd,
            os.SEEK_SET,
        )
        ...
        for fInd, f in enumerate(self.fieldsInRow()):
            ...
            val = (
                self.NoneValueStr
                if symInd < 0
                else self.getFieldVal(f["FieldName"], symInd)
            )  # NULL correction
            res[f["FieldName"]] = val
